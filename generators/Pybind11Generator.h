...
class Pybind11Generator : public CodeWriter {
  ...
  private:
    ...
    void write_numpy_helpers() {
        ...
        // Vector3 conversions
        line("py::array_t<double> vector3_to_numpy(const arch::Vector3& v) {");
        indent();
        ...
        line("arch::Vector3 numpy_to_vector3(py::array_t<double> arr) {");
        indent();
        line("if (arr.size() != 3) throw std::runtime_error(\"Expected array "
             "of size 3\");");
        line("auto buf = arr.unchecked<1>();");
        line("return arch::Vector3(buf(0), buf(1), buf(2));");
        dedent();
        line("}");
        line();
        ...
        line(
            "py::array_t<double> matrix33_to_numpy(const arch::Matrix33& m) {");
        indent();
    ...
    void write_class_binding(const std::string &name,
                             rosetta::ClassMetaHolder *holder) {
        line("// --- " + name + " ---");
